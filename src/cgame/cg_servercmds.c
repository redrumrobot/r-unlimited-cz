...
=================
*/
//static void CG_Say( int chatClientNum, int teamSelection, int clientNum, saymode_t mode, const char *text )//ROTAXfun
static void CG_Say( int clientNum, saymode_t mode, const char *text )//ROTAXfun
{
  char *name;
  char prefix[ 11 ] = "";
  char *ignore = "";
  ...
  switch( mode )
  {
    case SAY_ALL:
      ...
      /*
      //ROTAXfun
      if (!(cg.predictedPlayerState.stats[ STAT_TEAM ] == TEAM_NONE && chatClientNum == cg.clientNum))//svuj text ze specatoru nechci dat do bubliny
      {
        Q_strncpyz( cgs.clientinfo[ chatClientNum ].chatText, text, MAX_SAY_TEXT );
        cgs.clientinfo[ chatClientNum ].chatDurationTime = -1;//Start
        cgs.clientinfo[ chatClientNum ].chatTeam = TEAM_NONE;
      }
      */
      break;
    case SAY_TEAM:
      if ( clientNum == -2 )//ROTAXfun
        CG_Printf( "" S_COLOR_WHITE "%c" S_COLOR_CYAN "%s\n", INDENT_MARKER, text );
      else
...
static void CG_Chat_f( void )
{
  ...
  trap_Argv( 3, id, sizeof( id ) );//ROTAXfun
  trap_Argv( 4, mode, sizeof( mode ) );//ROTAXfun
  ...
  CG_Say( atoi( chatclientnum ), atoi( teamSelection ), atoi( id ), atoi( mode ), CG_Argv( 5 ) );//ROTAXfun
  */
