  ...
  if( old->team != new->team )
  {
    ...
    {//ROTAXFUN
      if ( old->team == TEAM_ALIENS )
        CG_Printf( "%s" S_COLOR_WHITE " left the ^1%ss^7\n", new->name, BG_TeamName( old->team ) );
      else
        CG_Printf( "%s" S_COLOR_WHITE " left the ^4%ss^7\n", new->name, BG_TeamName( old->team ) );
    }
    else if( old->team == TEAM_NONE )
    {//ROTAXFUN
      if ( new->team == TEAM_ALIENS )
        CG_Printf( "%s" S_COLOR_WHITE " joined the ^1%ss^7\n", new->name, BG_TeamName( new->team ) );
      else
        CG_Printf( "%s" S_COLOR_WHITE " joined the ^4%ss^7\n", new->name, BG_TeamName( new->team ) );
    }
    else
    {//ROTAXFUN
      if ( old->team == TEAM_ALIENS )
        CG_Printf( "%s" S_COLOR_WHITE " left the ^1%ss^7 and joined the ^4%ss^7\n", new->name, BG_TeamName( old->team ), BG_TeamName( new->team ) );
      else
        CG_Printf( "%s" S_COLOR_WHITE " left the ^4%ss^7 and joined the ^1%ss^7\n", new->name, BG_TeamName( old->team ), BG_TeamName( new->team ) );
    }
  }
}
...
static void CG_PlayerSprites( centity_t *cent )
{
  ...
  //rotaxfun
  //if (cgs.clientinfo[ cent->currentState.clientNum ].chatDurationTime > 0)
  //  return;
}
...
/*
==================
CG_ChatBubblleDisplay
==================
*/
static void CG_ChatBubblleDisplay( centity_t *cent )
{
  entityState_t   *es = &cent->currentState;
  vec3_t          origin;
  float           healthScale;
  ...
  if (cgs.clientinfo[ es->clientNum ].chatDurationTime == -1)
    cgs.clientinfo[ es->clientNum ].chatDurationTime = cg.time + 5000;

  if (cg.time > cgs.clientinfo[ es->clientNum ].chatDurationTime)
  {
    cgs.clientinfo[ es->clientNum ].chatDurationTime = 0;
    return;
  }
  ...
  origin[ 2 ] += BG_FindSpriteHeightForClass( ( cent->currentState.misc >> 8 ) & 0xFF );

  //entNum = cg.predictedPlayerState.clientNum;
 
  
  VectorCopy( cg.refdef.vieworg, trOrigin );
  CG_Trace( &tr, cg.refdef.vieworg, NULL, NULL, cent->lerpOrigin, cg.predictedPlayerState.clientNum, MASK_SHOT );
  //CG_Printf("a: %i, b: %i, wd: %i\n", tr.entityNum, cent->currentState.number, ENTITYNUM_WORLD);
  ...
  for( j = 0; j < 3; j++ )
  {
    ...
    switch( j )
    {
      case 1:
        ...
        VectorMA( trOrigin, 20, right, trOrigin );
        break;
      case 2:
        // step left
        AngleVectors( cg.refdefViewAngles, NULL, right, NULL );
...
*/
  ...
  health = 192 & ((1<<5)-1);
  healthScale = (float)health / ((1<<5)-1);
  ...
  if( CG_WorldToScreen( origin, &x, &y ) )
  {
    ...
    subY = picY + ( picH * 1.5f ) - ( subH * 1.5f );


      if( cgs.clientinfo[ cent->currentState.number ].chatTeam == TEAM_HUMANS )
        CG_DrawPic( picX, picY, picW, picH, cgs.media.chatBubblle_h );
      else if( cgs.clientinfo[ cent->currentState.number ].chatTeam == TEAM_ALIENS )
        CG_DrawPic( picX, picY, picW, picH, cgs.media.chatBubblle_a );
      else
        CG_DrawPic( picX, picY, picW, picH, cgs.media.chatBubblle );
...
//      trap_R_SetColor( NULL );
 ...
 //       nX -= subH * 1.5f;
     ...
     //CG_DrawField( nX, subY, 4, subH, subH, healthPoints );
      ...
      textW = UI_Text_Width( cgs.clientinfo[ es->clientNum ].chatText, scale );//, limit );//meri stretched sirku, takze priblizeny je vetsi
      nX = nX + ((picW / 2) - (textW / 2));
    
      UI_Text_Paint( nX, subY, scale, color, cgs.clientinfo[ es->clientNum ].chatText, 0, limit, ITEM_TEXTSTYLE_SHADOWED );
      //CG_Printf("textw: %i, nX: %i", textW, nX);
    }
    trap_R_SetColor( NULL );
  }
}
...
==================
*/
void CG_DrawChatBubblle( void )
{
  int             i;
  centity_t       *cent;
  entityState_t   *es;
  int             playerList[ MAX_ENTITIES_IN_SNAPSHOT ];//256
  int             players = 0;

  for( i = 0; i < MAX_GENTITIES; i++ )
  {
    if( cg_entities[ i ].currentState.eType == ET_PLAYER )
      playerList[ players++ ] = cg_entities[ i ].currentState.number;
       
      //CG_Printf( "etype: %i == %i, team: %i != %i\n", es->eType, ET_PLAYER, cg.predictedPlayerState.stats[ STAT_PTEAM ], PTE_NONE );
  }
...
/*
  for( i = 0; i < cg.snap->numEntities; i++ )
  {
    ...
    if( es->eType == ET_PLAYER && cg.predictedPlayerState.stats[ STAT_PTEAM ] != PTE_NONE )
      playerList[ players++ ] = cg.snap->entities[ i ].number;
  ...
  {
    if( cg_entities[ i ].currentState.eType == ET_PLAYER )
      CG_Printf( "etype: %i == %i, team: %i != %i\n", cg_entities[ i ].currentState.eType, ET_PLAYER, cg.predictedPlayerState.stats[ STAT_PTEAM ], PTE_NONE );
  }
   */ 
  qsort( playerList, players, sizeof( int ), CG_SortDistance );
  for( i = 0; i < players; i++ )
      CG_ChatBubblleDisplay( &cg_entities[ playerList[ i ] ] );
