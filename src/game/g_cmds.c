...
static qboolean G_SayTo( gentity_t *ent, gentity_t *other, saymode_t mode, const char *message )
{
  ...
  //ROTAXFUN
  //if (ent->client->pers.teamSelection == TEAM_NONE)//pokud specuju, nezobrazim tento text v bublinach
  //  clientNum = -2;
  //else
  //  clientNum = ent->client->ps.clientNum; 

  /*trap_SendServerCommand( other-g_entities, va( "chat %i %i %d %d \"%s\"",
    clientNum,
    ent->client->pers.teamSelection,//kvuli adminum aby ve specu videli barevne tymove bubliny, jinak by nesly rozlisit (potrebuju neco, co prozradi pravej tym i kdyz nekoho followuju)
...
void Cmd_CallVote_f( gentity_t *ent )
{
  ...
  else if( team == TEAM_NONE )
  {
    ...
    else
    {
      ...
      trap_SendServerCommand( ent-g_entities, "print \"Valid vote commands are: "
        "map, nextmap, map_restart, draw, sudden_death, kick, poll, mute and unmute\n" );
      return;
    }
  }
  ...
  if( team == TEAM_NONE )
  {
    trap_SendServerCommand( -1, va( "print \"^2____VOTE____^7 %s" S_COLOR_WHITE
    ...
    for( i = 0 ; i < level.maxclients ; i++ )
    {
      if( level.clients[ i ].pers.connected == CON_CONNECTED )
      {
        if( level.clients[ i ].pers.teamSelection == team ||
            ...
            G_admin_permission( &g_entities[ i ], ADMF_SPEC_ALLCHAT ) ) )
        {
          trap_SendServerCommand( i, va( "print \"^1____TEAMVOTE____^7 %s" S_COLOR_WHITE
...
void Cmd_Destroy_f( gentity_t *ent )
{
        ...
        ( ent->client->ps.weapon <= WP_HBUILD ) ) )
  {
    ...
    //rotax pokud nema jmeno, nemuze bourat
