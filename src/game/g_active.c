...
//    G_Printf("serverTime >>>>>\n" );
  ...
  //ROTAXfun
  if( BG_InventoryContainsUpgrade( UP_FLARE, client->ps.stats ) &&
      BG_UpgradeIsActive( UP_FLARE, client->ps.stats ) )
  {
    int lastWeapon = ent->s.weapon;

    //remove FLARE
    BG_DeactivateUpgrade( UP_FLARE, client->ps.stats );
    BG_RemoveUpgradeFromInventory( UP_FLARE, client->ps.stats );

    //M-M-M-M-MONSTER HACK
    ent->s.weapon = WP_FLARE;
    FireWeapon( ent );
    ent->s.weapon = lastWeapon;
  }
