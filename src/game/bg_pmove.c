...
static void PM_GroundClimbTrace( void )
{
  ...
  for( i = 0; i <= 4; i++ )
  {
    ...
    if( trace.fraction < 1.0f && !( trace.surfaceFlags & ( SURF_SKY | SURF_SLICK ) ) && ( trace.entityNum >= MAX_CLIENTS && i != 4) )//ROTAXfun
    {//if( trace.fraction < 1.0f && !( trace.surfaceFlags & ( SURF_SKY | SURF_SLICK ) ) && !( trace.entityNum != ENTITYNUM_WORLD && i != 4 ) )  
      if( i == 2 || i == 3 )
      {
        if( i == 2 )
...
static void PM_Weapon( void )
{
  ...
  else if( (pm->ps->weapon == WP_ALEVEL1 || pm->ps->weapon == WP_ALEVEL1_UPG) && attack3 )//ROTAXfun
  {
    //special case for acid bomb
    //if ( pm->ps->persistant[ PERS_CREDIT ] > 0 )
    //{
      pm->ps->ammo--;
